...
<!--
    <auto-generated>
        ...
        <description>
            If you'd like to regenerate the documentation, please open up powershell and run the following command :
            
            PS > .\psDoc.ps1 -moduleName ModuleName
...
<html lang="en">
<head>
    <title>[PS] [DOC] Powershell Documentation: $ModuleName</title>
    <meta charset="UTF-8">
...
$CommandsHelp | % {
                    ...
                    $Progress++;
@"
			    <div id=`"$(Convert-HTMLEntities($_.Name))`" class="toggle_container">
				    <div class="page-header">
					    <h2>$(Convert-HTMLEntities($_.Name))</h2>
"@
$Syn = Convert-HTMLEntities($_.synopsis);
if (!($Syn).StartsWith($(Convert-HTMLEntities($_.Name)))) {
@"
					    <p>$Syn</p>
					    <p>$(Convert-HTMLEntities(($_.Description | Out-String).Trim()) $True)</p>
"@
}
@"
				    </div>
...
@"
					    ...
					    <div class="panel panel-default">
                            <div class='panel-body'>
                                <pre class="brush: ps">$(Convert-HTMLEntities($_.syntax | Out-String))</pre>
                            </div>
					    </div>
"@
}
...
"@
	    $_.examples.example | % {
@"
							    <strong>$(Convert-HTMLEntities($_.title.Trim(('-',' '))))</strong>
    <pre class="brush: ps">$(Convert-HTMLEntities($_.code | Out-String ).Trim())</pre>
							    <div>$(Convert-HTMLEntities($_.remarks | Out-String ).Trim())</div>
"@
	    }
@"
                            </div>
					    </div>
                        <p class='pull-right'><a onclick='document.body.scrollTop = document.documentElement.scrollTop = 0;' style='cursor: pointer;'>top ^</a>
"@
    }
